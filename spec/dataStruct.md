...
本数据结构格式描述中的每一个数值定义与约束均使用[valueFormat](./valueFormat.md) 约定的方式定义
...
### 数值格式的约定
```
	数值格式的约定使用 [API接口的参数格式描述方式](valueFormat.md) 为标准
```
...
### 可接受未设置的参数
```
格式表中出现 key 为 "..." 时，表示本级参数表接受未在参数标准明确标出的 key .
使用这个扩展 key 的功能存在风险，参数表中无法对未知的参数值进行格式设定，因此也无法对值的有效范围进行检查.
key 为 "..." 时，值的格式可以统一为一种格式。可以统一为 string ，即不做格式检查
```
    ...
    示例如下:
	{
		...
		"carData":{
		    ...
		    "extraDatas":{
				...
				"...":"mobile//这里虽然扩展了key,但所有的扩展值都要符合mobile的格式"
		    	}
		}
	}
